...
{
  /* Configure SysTick to standard 1ms period */
  SysTick->LOAD  = (uint32_t)((SystemCoreClock / 1000ULL) - 1ULL);
  SysTick->VAL   = 0UL;
  /* Enable SysTick counter, but disable interrupt */
...
uint32_t HAL_GetTick(void)
{
  ...
  if (load > 0)
  {
    if (val > last_val)
    {
      accumulated_ticks += (load - val) + last_val;
    }
    else
    {
      accumulated_ticks += last_val - val;
    }
    last_val = val;
    uwTick = accumulated_ticks / (SystemCoreClock / 1000UL);
  }
  else
  {
    /* Fallback dummy ticks to prevent hanging on early calls */
    static uint32_t dummy_ticks = 0;
    uwTick = ++dummy_ticks;
  }
