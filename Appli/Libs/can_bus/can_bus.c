...
uint16_t CAN_Bus_makeID(uint8_t deviceId, CAN_BusMessageId_t messageId,
                        CAN_BusPriority_t priority) {
  return ((((priority) & 0x7U) << 8) | (((messageId) & 0xFU) << 4) |
          ((deviceId) & 0xFU));
}

static void CAN_Bus_MessageInit(CAN_BusMessage_t *message, uint8_t deviceId,
...
static void CAN_Bus_BuildU8(CAN_BusMessage_t *message, uint8_t deviceId,
                            CAN_BusMessageId_t messageId,
                            CAN_BusPriority_t priority, uint8_t value) {
  if (message == NULL) {
    return;
  }
...
static void CAN_Bus_BuildU16(CAN_BusMessage_t *message, uint8_t deviceId,
                             CAN_BusMessageId_t messageId,
                             CAN_BusPriority_t priority, uint16_t value) {
  if (message == NULL) {
    return;
  }
...
static void CAN_Bus_BuildU32(CAN_BusMessage_t *message, uint8_t deviceId,
                             CAN_BusMessageId_t messageId,
                             CAN_BusPriority_t priority, uint32_t value) {
  if (message == NULL) {
    return;
  }
...
static HAL_StatusTypeDef CAN_Bus_SendMessage(FDCAN_HandleTypeDef *hfdcan,
  ...
  if ((hfdcan == NULL) || (message == NULL) || (message->dlc > 8U) ||
      (message->deviceId > 0x0FU) || (message->messageId > 0x0FU) ||
      (message->priority > 0x07U)) {
    return HAL_ERROR;
  }

  tx_header.Identifier =
      CAN_Bus_makeID(message->deviceId, message->messageId, message->priority);
  ...
  //    printf("CAN_TX [ID: 0x%03lX] DLC: %lu Data: ", (unsigned
  //    long)tx_header.Identifier, (unsigned long)message->dlc); for (uint8_t i
  //    = 0; i < message->dlc; i++) {
  //        printf("%02X ", message->data[i]);
  //    }
  //    printf("\r\n");

  return HAL_FDCAN_AddMessageToTxFifoQ(hfdcan, &tx_header,
                                       (uint8_t *)message->data);
}

HAL_StatusTypeDef CAN_Bus_SendU8(FDCAN_HandleTypeDef *hfdcan, uint8_t deviceId,
                                 CAN_BusMessageId_t messageId,
                                 CAN_BusPriority_t priority, uint8_t value) {
...
HAL_StatusTypeDef CAN_Bus_SendU16(FDCAN_HandleTypeDef *hfdcan, uint8_t deviceId,
                                  CAN_BusMessageId_t messageId,
                                  CAN_BusPriority_t priority, uint16_t value) {
  CAN_BusMessage_t message;
...
HAL_StatusTypeDef CAN_Bus_SendU32(FDCAN_HandleTypeDef *hfdcan, uint8_t deviceId,
                                  CAN_BusMessageId_t messageId,
                                  CAN_BusPriority_t priority, uint32_t value) {
  CAN_BusMessage_t message;
...
HAL_StatusTypeDef CAN_Bus_Receive(FDCAN_HandleTypeDef *hfdcan,
                                  CAN_BusMessage_t *message) {
  ...
  message->messageId =
      (CAN_BusMessageId_t)((rx_header.Identifier >> 4) & 0x0FU);
...
uint32_t CAN_Bus_ReadU32(const CAN_BusMessage_t *message) {
  ...
  return (uint32_t)message->data[0] | ((uint32_t)message->data[1] << 8) |
