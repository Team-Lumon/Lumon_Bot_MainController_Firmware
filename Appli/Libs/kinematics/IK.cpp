...
extern "C" void initGeometry() {
  ...
  const float hx = 0.05f;   // half of 0.1m width
  const float hy = 0.0625f; // half of 0.125m length
...
void computeFrameTargets(float t, const Vector3f &r_start,
  ...
  static Matrix<float, 4, 1> last_good_tau =
      Matrix<float, 4, 1>::Constant((T_MIN + T_MAX) / 2.0f);
  ...
  Eigen::VectorXd g0 = -25.0 * Eigen::VectorXd::Ones(4);

  Eigen::MatrixXd CE = W.transpose().cast<double>();
  Eigen::VectorXd ce0 = -f_dyn.cast<double>();

  Eigen::MatrixXd CI(4, 8);
  CI.leftCols(4) = Eigen::MatrixXd::Identity(4, 4);
  CI.rightCols(4) = -Eigen::MatrixXd::Identity(4, 4);

  Eigen::VectorXd ci0(8);
  ci0.head(4) = -T_MIN * Eigen::VectorXd::Ones(4);
  ci0.tail(4) = T_MAX * Eigen::VectorXd::Ones(4);
