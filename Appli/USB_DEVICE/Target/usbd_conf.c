...
/* Private variables ---------------------------------------------------------*/
/* USER CODE BEGIN PV */
uint32_t HAL_RCC_USBPHYC_CLK_ENABLED = 0;
/* USER CODE END PV */
...
void HAL_PCD_MspInit(PCD_HandleTypeDef *pcdHandle) {
  ...
  if (pcdHandle->Instance == USB_OTG_FS) {
    ...
    printf("USB MSP Init (OTG HS)... \r\n");
    /* USER CODE END USB_OTG_HS_MspInit 0 */

    /** Enable USB Voltage detector and HS Regulator */
    printf("Enabling USB Voltage Detector & HS Regulator...\r\n");
    HAL_PWREx_EnableUSBVoltageDetector();
    HAL_PWREx_EnableUSBHSregulator();

    printf("Waiting for USB 3.3V supply to become ready...\r\n");
    while (READ_BIT(PWR->CSR2, PWR_CSR2_USB33RDY) == 0U) {
      // Wait for supply stability
    }
    printf("USB 3.3V supply ready.\r\n");

    /* Step 1: Enable peripheral clocks */
    printf("Enabling USB_OTG_HS and USBPHYC clocks...\r\n");
    ...
    /* Step 2: Reset hardware blocks BEFORE writing clock config */
    printf("Resetting USB HS & USBPHYC hardware blocks...\r\n");
    __HAL_RCC_USB_OTG_HS_FORCE_RESET();
    __HAL_RCC_USB_OTG_HS_RELEASE_RESET();
    __HAL_RCC_USBPHYC_FORCE_RESET();
    __HAL_RCC_USBPHYC_RELEASE_RESET();

    /* Step 3: Configure clock source AFTER reset (so it sticks) */
    printf("Configuring USBPHYC clock source and 24MHz reference...\r\n");
    MODIFY_REG(RCC->CCIPR1, (RCC_CCIPR1_USBPHYCSEL | RCC_CCIPR1_USBREFCKSEL), 
               (RCC_CCIPR1_USBREFCKSEL_3 | RCC_CCIPR1_USBREFCKSEL_1));
    printf("USBPHYC clock source configured.\r\n");

    /* Step 4: Power ON the HS analog transceiver BEFORE USB_CoreInit runs */
    USB_OTG_HS->GCCFG |= USB_OTG_GCCFG_PWRDWN;

    /* Step 5: Enable NVIC interrupt */
    HAL_NVIC_SetPriority(OTG_HS_IRQn, 0, 0);
    HAL_NVIC_EnableIRQ(OTG_HS_IRQn);

    /* Step 6: Wait for PHY PLL to stabilize */
    printf("Waiting 50ms for PHY PLL to stabilize...\r\n");
    HAL_Delay(50);
    printf("PHY ready.\r\n");
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev) {
  ...
  if (pdev->id == DEVICE_FS) {
    ...
    hpcd_USB_OTG_HS.Init.speed =
        PCD_SPEED_HIGH_IN_FULL; // High-Speed PHY running in Full-Speed mode
    ...
    /* Note: GCCFG_PWRDWN is set inside HAL_PCD_MspInit before USB_CoreInit runs */
